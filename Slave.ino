...
void setup() {
  ...
  radio.begin();
  radio.openWritingPipe(address[0]);    // Отправка на Узел 1
  radio.openReadingPipe(1, address[1]); // Прием от Узла 1
...
void loop() {
  ...
  // Отправляем свою температуру Узлу 1
  if (received) {
    radio.stopListening();
  ...
  // Выводим данные
  Serial.print("Slave: Собственная температура: ");
  Serial.print(ownTemp);
  Serial.print("°C, Температура Master: ");
  ...
  if (averageTemp > TEMP_THRESHOLD) {
    ...
    Serial.println("Slave: Температура превышает порог! Открываю форточку.");
  } else {
...
float readTemperature() {
  ...
  // Пересчет сопротивления в температуру (упрощенная формула)
